  ...
  switch (command) {
    ...
    case 'knowledge': {
      ...
      if (sub === 'lint') {
        ...
        for (const f of malformed) console.error(`broken : ${f.detail}`);
      ...
      if (sub === 'unhook') {
        ...
        for (const warning of r.warnings) console.error(warning);
    ...
    case 'init': {
      ...
      if (!has('--no-sync')) {
        if (resolveAdapters(resolve(dir)).source === 'none') {
          ...
          for (const w of sres.written) console.error(`  ${w}`);
