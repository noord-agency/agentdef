...
export interface ValidationIssue {
  level: 'error' | 'warning';
  message: string;
  // Printed once per distinct value after the issue list, not per issue: nine
  // broken knowledge docs are nine lines but one remedy, and repeating the
  // remedy nine times is how a fixable failure starts reading as noise.
  hint?: string;
}
...
export function validate(dir: string): ValidationIssue[] {
  ...
  try {
    const include = parseIncludeList((manifest as { include?: unknown }).include, 'agent.yaml');
    ...
    issues.push({ level: 'error', message: `agent.yaml: ${(e as Error).message}` });
  }
