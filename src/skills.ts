...
export function getAllowedTools(frontmatter: Record<string, unknown>): string[] {
...
export function parseSkillMd(filePath: string): SkillFull {
  const content = readFileSync(filePath, 'utf-8');
  const match = content.match(/^---\n([\s\S]*?)\n---\n*([\s\S]*)$/);
  if (!match) {
    throw new Error(`SKILL.md at ${filePath} is missing YAML frontmatter (---)`);
...
function eachSkill<T>(skillsDir: string, parse: (p: string) => T): T[] {
