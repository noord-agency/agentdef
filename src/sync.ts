...
function parseAdapterList(text: string): string[] {
  return text
    .split('\n')
    .map((l) => l.trim())
...
export function machineAdaptersPath(): string {
  if (process.env.AGENTDEF_ADAPTERS_FILE) return process.env.AGENTDEF_ADAPTERS_FILE;
...
export function resolveAdapters(agentDir: string, override?: string[]): ResolvedAdapters {
  if (override && override.length) return { adapters: override, source: 'flag' };

  const local = join(agentDir, '.agent-adapters');
  if (existsSync(local)) {
    const list = parseAdapterList(readFileSync(local, 'utf-8'));
    if (list.length) return { adapters: list, source: 'repo', path: local };
  }

  const machine = machineAdaptersPath();
  if (existsSync(machine)) {
    const list = parseAdapterList(readFileSync(machine, 'utf-8'));
    if (list.length) return { adapters: list, source: 'machine', path: machine };
  }
...
export function writeAdapters(tools: string[], opts: { local: boolean; dir: string }): { path: string; unknown: string[] } {
...
function readAdapters(agentDir: string, override?: string[]): string[] {
  ...
  if (r.adapters.length) return r.adapters;
