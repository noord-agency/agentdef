...
export function knowledgeHookEnabled(agentDir: string): boolean {
  if (!existsSync(join(agentDir, 'agent.yaml'))) return true;
...
export function renderKnowledgeDigest(
  entries: KnowledgeMetadata[],
  opts: { indexPath: string },
): string {
  const counts = new Map<string, number>();
  for (const doc of entries) counts.set(doc.type, (counts.get(doc.type) ?? 0) + 1);
